...
5. **`data-step` 是 SVG 累积演进的"保留属性"，不要给 SVG 以外的元素加**：CSS 里 `[data-step] { opacity: 0 }` 是全局选择器，任何带 `data-step` 的 DOM（包括动态新建的侧栏圆点、控件等）都会默认隐形，需要被 `goTo` 里的 `querySelectorAll('[data-step]')` toggle 才会显形。但 `goTo` 的 toggle 一般在 `renderDots`/`renderExtras` 之前跑，**后创建的元素拿不到 `.visible` 类，永远隐形**。两条对策：① 给 SVG 以外的元素用 `data-step-num` / `data-svg-step` 这种别名；② 如非要用 `data-step` 就把 toggle 移到 `goTo` 末尾、所有 render 之后。VPN 专题侧栏圆点踩过一次（commit `c75b78d`），表现是"侧栏完全没有进度指示"。
