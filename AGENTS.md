...
- `index.html`（仓库根）— landing 页面，并列展示两条路径。
...
- **视觉主题**：cream 底（`#fdfaf6`）+ cherry red 主调（`#aa1141`）。所有调色板变量定义在 `:root`，新代码必须复用 `var(--cherry / --teal / --amber / --green / --violet / --slate)`。
...
- `diagrams/index.html` 顶部那块 🐣 CTA 卡片链接到 `../diagrams-beginner/`，让从老手版进入的读者也能切到新手版。仓库根 `index.html` 是统一入口。
- 视觉规范与渐进版一致；每张图额外有底部「📚 基础知识速查 · Reference」卡。
